...
class CharacterService {
  ...
  async findByIdIndex(id: number) {
    const index = await this.findByIdIndex(id);
    return ({ status: 200, data: index });
  }

  async update(id: number, character: Character) {
    const updatedCharacter = await this.update(id, character);
    return ({ status: 200, data: updatedCharacter });
  }

  async delete(id: number) {
    const deletedCharacter = await this.delete(id);
  ...
  async getById(id: number) {
    const findCharacterById = await this.getById(id);
    return ({ status: 200, data: findCharacterById });
  }
}
